...
**여러 축에서 동시에 이기는 것**을 목표로 만든 손실 이미지 코덱입니다. 압축률 하나만 보지 않습니다. 이것이 **v1.1** 이고, 포맷은 여기서 고정됐습니다. 아래 숫자가 그 고정의 근거입니다.

[English](README.md)

대부분의 코덱은 한 성질을 얻으려고 다른 성질을 내줍니다. ingot 은 아무도 최적화하지 않아 비어 있던 축을 골라, 그것을 포맷 자체에 못박았습니다.

- **JPEG 을 세 지표 모두에서 앞섭니다** — 표준 사진에서 BD-rate **−23.3%**(PSNR), **−21.1%**(SSIM), **−11.3%**(SSIMULACRA2) 입니다. WebP 는 PSNR 로 **−4.4%** 앞서고, JPEG XL 과는 PSNR 이 같습니다.
...
- **작습니다** — `src/` 의 C99 코드가 2,030 줄이고 그중 1,303 줄이 디코딩 경로입니다. libc 말고는 의존하는 것이 없습니다.
...
이 소재에서는 JPEG 을 세 지표 모두에서, WebP 와 JPEG XL 을 셋 중 둘에서 앞섭니다.
...
| 단순성 | **예산 안, 다만 누구와도 겨룬 적 없음** | 디코딩 경로 1,303 줄, 전체 2,030 줄, 외부 의존 0, 규격 12 쪽 이내. 이 축에서 상대를 재본 적이 없습니다 |
| 특허 | 만료된 기술만 | DCT(1974), 골롬(1966), 산술 부호화(1970년대 후반). **법률 검토는 아직 안 했습니다** |
| 디코딩 속도 | **다섯 중 가장 느림** | 시계로는 0.096 초이고 JPEG 0.065, WebP 0.076, JXL 0.088, AVIF 0.100 입니다. 그런데 프로세스가 시작만 하는 데 ffmpeg 는 0.059 초, 우리 CLI 는 0.028 초가 듭니다. 그것을 빼면 실제로 푸는 시간이 우리 0.068 초, JPEG 0.006, WebP 0.017, JXL 0.029, AVIF 0.041 입니다. 우리가 꼴찌이고 JPEG 의 11 배입니다 |
