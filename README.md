...
A lossy image codec built to win on **several axes at once** — not just compression ratio. This is **v1.12**.
...
- **Ahead of JPEG, WebP and JPEG XL on all three metrics** — against JPEG **−39.6%** (PSNR), **−40.8%** (SSIM), **−33.2%** (SSIMULACRA2) on standard photos; against WebP **−26.7%**, **−19.2%**, **−24.6%**; against JPEG XL **−23.4%**, **−10.8%**, **−1.5%**. Against AVIF we are ahead on the perceptual metric by **−7.0%** and behind on the two squared-error ones by **+18.1%** and **+12.0%**.
...
**AVIF is still ahead on both squared-error metrics, and the gap is 18.1%.** That is not a rounding difference — it is roughly what a mature encoder with dozens of directional prediction modes and a transform-type search buys. Measured per quality step, the gap is 1.11–1.25× at PSNR 30 and 1.3–1.96× at PSNR 40: we lose most at high quality, on flat images. The header — split flags, mode symbols, the last-coefficient position — is 13–16% of the file on those images and 4.6% on the ones where we do best.
...
| Binary arithmetic coding (range coder) | −1.0% → **−16.7%** | +7.9% → **−10.8%** |
| Neighbor magnitude in the coefficient context | −16.7% → **−17.6%** | −10.8% → **−10.9%** |
...
| Trying all four modes instead of the top two (v1.7, encoder only) | **−1.5%** | −1.7% |
| Not coding a flag whose answer is fixed (v1.7) | **−4.9%** | **−4.3%** |
| Block boundary filter (v1.7, decoder only, costs zero bits) | **−7.2%** | **−7.0%** |
