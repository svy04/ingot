...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/C99-6%2C980%20lines-087ea3" alt="6,980 lines of C99">
  <img src="https://img.shields.io/badge/deps-libc%20only-5d6e73" alt="Zero dependencies">
</p>

A lossy image codec built to win on **several axes at once** — not just compression ratio. This is **v1.11**.
...
- **Ahead of JPEG, WebP and JPEG XL on all three metrics** — against JPEG **−39.1%** (PSNR), **−40.7%** (SSIM), **−33.7%** (SSIMULACRA2) on standard photos; against WebP **−26.1%**, **−18.6%**, **−24.8%**; against JPEG XL **−22.7%**, **−10.2%**, **−2.0%**. Against AVIF we are ahead on the perceptual metric by **−7.7%** and behind on the two squared-error ones by **+19.3%** and **+12.8%**.
...
| JPEG | **−39.1%** | **−40.7%** | **−33.7%** |
| WebP | **−26.1%** | **−18.6%** | **−24.8%** |
| JPEG XL | **−22.7%** | **−10.2%** | **−2.0%** |
| AVIF | +19.3% | +12.8% | **−7.7%** |

**AVIF is still ahead on both squared-error metrics, and the gap is 19.3%.** That is not a rounding difference — it is roughly what a mature encoder with dozens of directional prediction modes and a transform-type search buys. Measured per quality step, the gap is 1.11–1.25× at PSNR 30 and 1.3–1.96× at PSNR 40: we lose most at high quality, on flat images. The header — split flags, mode symbols, the last-coefficient position — is 13–16% of the file on those images and 4.6% on the ones where we do best.
...
| Binary arithmetic coding (range coder) | −1.0% → **−16.7%** | +7.9% → **−10.2%** |
| Neighbor magnitude in the coefficient context | −16.7% → **−17.6%** | −10.2% → **−10.9%** |
...
| Trying all four modes instead of the top two (v1.7, encoder only) | **−2.0%** | −1.7% |
