...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/C99-7%2C339%20lines-087ea3" alt="7,339 lines of C99">
  <img src="https://img.shields.io/badge/deps-libc%20only-5d6e73" alt="Zero dependencies">
</p>

A lossy image codec built to win on **several axes at once** — not just compression ratio. This is **v1.13**.
...
- **Ahead of JPEG, WebP and JPEG XL on all three metrics** — against JPEG **−40.7%** (PSNR), **−42.3%** (SSIM), **−34.4%** (SSIMULACRA2) on standard photos; against WebP **−27.9%**, **−20.9%**, **−25.9%**; against JPEG XL **−24.9%**, **−13.3%**, **−3.4%**. Against AVIF we are ahead on the perceptual metric by **−9.1%** and behind on the two squared-error ones by **+14.9%** and **+7.9%**.
...
| JPEG | **−40.7%** | **−42.3%** | **−34.4%** |
| WebP | **−27.9%** | **−20.9%** | **−25.9%** |
| JPEG XL | **−24.9%** | **−13.3%** | **−3.4%** |
| AVIF | +14.9% | +7.9% | **−9.1%** |

**AVIF is still ahead on both squared-error metrics, and the gap is 14.9%.** That is not a rounding difference — it is roughly what a mature encoder with dozens of directional prediction modes and a transform-type search buys. Measured per quality step, the gap is 1.11–1.25× at PSNR 30 and 1.3–1.96× at PSNR 40: we lose most at high quality, on flat images. The header — split flags, mode symbols, the last-coefficient position — is 13–16% of the file on those images and 4.6% on the ones where we do best.
...
| Binary arithmetic coding (range coder) | −1.0% → **−16.7%** | +7.9% → **−13.3%** |
| Neighbor magnitude in the coefficient context | −16.7% → **−17.6%** | −13.3% → **−10.9%** |
...
| Per-size context for the block header | −24.2% → **−25.9%** | −19.7% → **−20.1%** |
| Split early-exit in the encoder | −25.9% → **−25.6%**, and 1.6× faster | −20.1% → **−21.0%** |
...
| Trying all four modes instead of the top two (v1.7, encoder only) | **−3.4%** | −1.7% |
| Not coding a flag whose answer is fixed (v1.7) | **−4.9%** | **−4.3%** |
| Block boundary filter (v1.7, decoder only, costs zero bits) | **−7.2%** | **−9.1%** |
...
| Directional deringing filter, decoder only, costs zero bits (v1.11) | **−0.61%** | **−0.44%** |
| The same filter one strength step higher | **+0.15%** | **+0.75%, SSIMULACRA2 +1.67% (not taken)** |
| 128×128 blocks | **(not taken; encode time past the point of measuring)** | |
| Dual self-guided restoration, weights solved by least squares (v1.12) | **−0.71%** | **−0.40%, SSIMULACRA2 +0.65%** |
...
Chroma-from-luma lost twice — by 77% the first time and by 3.8 dB the second — with the same signature both times: **the file gets 4% smaller while the picture gets worse.** That combination has one meaning. The encoder is choosing a prediction the decoder then builds differently, so the residual really is smaller on the encoder side and the bytes really do go down.

The bug was not in the prediction. The decoder filters the luma plane, then decodes chroma against the filtered result; the encoder never filters its own reconstruction buffer. Turning the filters off one at a time is what found it — with all filters off the tool wins, with the deblocking filter alone it loses 7.4 dB, with the deringing filter alone 8.6 dB, and with the guided filter alone it wins, because that one runs after the conversion to RGB and so lands *after* chroma prediction has already happened. The fix is one extra argument to the plane decoder: where to copy the pre-filter luma. It is worth **−1.73% / −2.65% / −1.96%** and it is the largest single change of the day. Two earlier attempts to fix it by guessing made it worse. **Isolate before you repair.**
...
src/deringe.c      directional deringing filter (decoder side)
src/guided.c       dual self-guided restoration, weights solved per group
src/dct128.c       128-point transform tables, behind a knob that is off
